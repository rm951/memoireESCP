...
def analyze_budget_data(date_analysis):
    ...
    columns_to_delete = df.columns[df.columns.str.startswith(
        'C13') | df.columns.str.startswith('C14')]
    ...
    df_inflation_raw = df_inflation_raw.rename(
        columns={'geo\TIME_PERIOD': 'geo'})
    df_correspondances = pd.read_csv(
        "correspondances.csv", dtype="str", sep=";")
    df_correspondances.set_index('colonnes de df_prop', inplace=True)
    df_inflation = df_inflation_raw[df_inflation_raw['coicop'].str.startswith(
        'CP')]
    ...
    df_inflation_by_household = df_inflation_by_household.rename(columns={
                                                                 0: "inflation"})

    # Saving results to a CSV file
    print("Saving results to a CSV file...")
    df_inflation_by_household.to_csv(
        f"BDF/computed_inflation_by_household_{date_analysis}.csv")

    print(f"Analysis for {date_analysis} completed.")


dates = pd.date_range(start="2023-05", end="2023-06", freq='M')
